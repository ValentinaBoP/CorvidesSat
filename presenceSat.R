...
# order species
levels = rev(c("corCor", "corBra", "corWoo", "corMon", "corSpl", "corDau", "cinMag", "cinReg", "parRag", "parRub", "astSte", "astRot", "parBre", "dreAlb", "epiMey", "ptiInt", "ptiMag", "parHel", "parLaw", "manKer", "manCha", "lycPyr"))
...
plot = ggplot(data = df, aes(x = Satellite, y = Species)) + geom_tile(aes(fill = Proportion)) + scale_fill_gradient(low = "white", high = "red") + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1))
